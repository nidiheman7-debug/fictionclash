...
// Bumped to v2 to force every existing install to drop its old cached
// shell (stale index.html/manifest.json were being served over fresh
// deploys — see the network-first change below for why that stops
...
// even is (install button logic, icon references, etc.) — serving a
// stale cached copy of these after a deploy is what caused "I fixed it
// but it's still broken" symptoms.
...
self.addEventListener('fetch', event => {
  ...
  if (isAlwaysFreshRequest(event.request, url)) {
    // Network-first: always get the latest shell when online. Only touch
    // the cache as an offline fallback, never as the primary answer.
    event.respondWith(
      fetch(event.request)
        .then(response => {
  ...
  // Everything else (icons, fonts, etc.): cache-first is fine — these
  // don't change the app's behavior, so serving instantly from cache
  // while updating it in the background is a safe speed win.
  event.respondWith(
